...
- 创建 8 张数据表
- 插入默认管理员和普通用户
- 插入大陆 31 个省级地区 2015-2024 年初始化人口数据
...
当前初始化数据覆盖大陆 31 个省级地区 `2015-2024` 年，共 310 条人口年度核心记录。

数据口径统一按官方年鉴来源整理和折算：总人口、城镇化率采用《中国统计年鉴2025》人口章节 2-5、2-6 的十年序列表；2024 年地区出生率、死亡率、性别结构和年龄结构采用《中国统计年鉴2025》人口章节 2-7、2-9、2-11；历史年份出生率、死亡率、性别人口和老龄化率按年鉴样本结构及全国年度趋势折算，并通过 `data_quality=official_yearbook_series_derived`、`aging_rate_basis=65_plus` 和 `remarks` 标注来源与派生口径。

数据库初始化会对大陆 31 个省级地区 `2015-2024` 的官方种子记录执行 upsert，确保已有 SQLite 数据库中的同地区同年份旧样例记录会被统一口径覆盖。
